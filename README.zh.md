...
`remotelab start` / `remotelab stop` 会自动识别系统：
- macOS 走 `launchctl`
- Linux 走 `systemctl --user`

如果 Linux 提示缺少 `remotelab-chat` / `remotelab-proxy` 服务，请先执行一次 `remotelab setup` 生成 systemd user services。

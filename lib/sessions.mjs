...
function findTtydPath() {
  try {
    ...
    // Fallback common locations (macOS Homebrew + Linux system paths)
    const candidates = process.platform === 'darwin'
      ? ['/opt/homebrew/bin/ttyd', '/usr/local/bin/ttyd']
      : ['/usr/local/bin/ttyd', '/usr/bin/ttyd', '/snap/bin/ttyd'];
...
function findWrapperPath() {
  ...
  const candidates = [
    ...
    join(dirname(new URL(import.meta.url).pathname), '..', 'scripts', 'claude-ttyd-session'),
  ];
  for (const p of candidates) {
    if (existsSync(p)) return p;
  }
...
export async function spawnSessionTtyd(session) {
  ...
  const args = [
    '--writable',
    '--port', String(port),
    '--interface', process.platform === 'darwin' ? 'lo0' : 'lo',
    '--ping-interval', '300',
    '--max-clients', '0',
